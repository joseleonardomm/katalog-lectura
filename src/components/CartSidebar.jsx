...
import { FaTrashAlt, FaTimes, FaCreditCard, FaCopy, FaCheck, FaChevronDown, FaChevronRight } from 'react-icons/fa';
...
function CartSidebar({ isOpen, onClose, cart, onUpdateQuantity, onRemove, onClear, formatPrice, total, config }) {
  ...
  // 🔒 Bloqueo robusto del scroll del fondo
  useEffect(() => {
    if (isOpen) {
  ...
  useEffect(() => {
    const loadFormConfig = async () => {
      ...
      setOrderForm(cfg);
      const initialData = {};
      if (cfg.fields) cfg.fields.forEach(field => { initialData[field.label] = ''; });
  ...
  useEffect(() => {
    ...
    cart.forEach(item => {
      ...
      if (catId && zone.extras && zone.extras[catId]) extraTotal += zone.extras[catId] * item.quantity;
    });
  ...
  return (
    <>
      {/* Overlay oscuro que separa visualmente el carrito del fondo */}
      ...
      <div className={`cart-sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div className="cart-scrollable">
          ...
          {cart.length > 0 && (
            <>
              {shipping.enabled && (
                <div className="cart-section cart-shipping">
                  <button className="cart-section-toggle" onClick={() => setShippingOpen(!shippingOpen)}>
                    <span>🚚 Envío</span>
                    ...
                    <div className="cart-section-body">
                      ...
                      {selectedZoneId && shippingInfo && (
                        <div className="shipping-details">
                          ...
                          <p><strong>Costo:</strong> {shippingCost === 0 ? '¡Gratis!' : shippingInfo.min === shippingInfo.max ? formatPrice({ price: shippingInfo.min, priceCurrency: cart[0]?.product.priceCurrency || 'USD' }) : `${formatPrice({ price: shippingInfo.min, priceCurrency: cart[0]?.product.priceCurrency || 'USD' })} – ${formatPrice({ price: shippingInfo.max, priceCurrency: cart[0]?.product.priceCurrency || 'USD' })}`}</p>
                        </div>
                      )}
                    </div>
                  )}
              ...
              {activePaymentMethods.length > 0 && (
                <div className="cart-section cart-payment-methods">
                  <button className="cart-section-toggle" onClick={() => setShowPaymentMethods(!showPaymentMethods)}>
                    <span><FaCreditCard /> Métodos de pago</span>
                    {showPaymentMethods ? <FaChevronDown /> : <FaChevronRight />}
                  </button>
                  {showPaymentMethods && (
                        ...
                        <div key={method.id} className="payment-method-item">
                          <div className="payment-method-header">
                            <span className="payment-method-type">
                              {method.type === 'pago_movil' ? '📱 Pago Móvil' : method.type === 'transferencia' ? '🏦 Transferencia' : method.type === 'paypal' ? '🅿️ PayPal' : '💳 Otro'}
                            </span>
                          ...
                          <div className="payment-method-details">
                            {method.type === 'pago_movil' || method.type === 'transferencia' ? (
                              <>
                                {method.details.bank && (
                                  <div className="payment-detail-row">
                                    ...
                                    <button className="btn-copy-icon" onClick={() => { navigator.clipboard.writeText(method.details.bank); setCopiedIndex(`${idx}-bank`); setTimeout(() => setCopiedIndex(null), 2000); }} title="Copiar banco">
                                ...
                                {method.details.phone && (
                                  <div className="payment-detail-row">
                                    ...
                                    <button className="btn-copy-icon" onClick={() => { navigator.clipboard.writeText(method.details.phone); setCopiedIndex(`${idx}-phone`); setTimeout(() => setCopiedIndex(null), 2000); }} title="Copiar teléfono">
                                ...
                                {method.details.idNumber && (
                                  <div className="payment-detail-row">
                                    ...
                                    <button className="btn-copy-icon" onClick={() => { navigator.clipboard.writeText(method.details.idNumber); setCopiedIndex(`${idx}-id`); setTimeout(() => setCopiedIndex(null), 2000); }} title="Copiar cédula">
                              ...
                              <div className="payment-detail-row">
                                <a href={method.details.paypalLink} target="_blank" rel="noopener noreferrer" className="paypal-link">Ir a PayPal</a>
                                <button className="btn-copy-icon" onClick={() => { navigator.clipboard.writeText(method.details.paypalLink); setCopiedIndex(`${idx}-paypal`); setTimeout(() => setCopiedIndex(null), 2000); }} title="Copiar enlace">
                              ...
                              <div className="payment-detail-row">
                                ...
                                <button className="btn-copy-icon" onClick={() => { navigator.clipboard.writeText(method.details.customText); setCopiedIndex(`${idx}-custom`); setTimeout(() => setCopiedIndex(null), 2000); }} title="Copiar información">
                                  {copiedIndex === `${idx}-custom` ? <FaCheck /> : <FaCopy />}
                                </button>
                              </div>
                            )}
                          </div>
                          {method.instructions && <p className="payment-method-instructions">{method.instructions}</p>}
                        </div>
                      ))}
                      <div className="payment-reminder"><p>💡 Recuerda enviar tu comprobante de pago al WhatsApp de la tienda después de realizar el pago.</p></div>
                    </div>
                  )}
                </div>
              )}
              ...
              {orderForm.enabled && (
                <div className="cart-section cart-contact">
                  <button className="cart-section-toggle" onClick={() => setContactOpen(!contactOpen)}>
